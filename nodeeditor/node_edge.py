...
class Edge(Serializable):
    ...
    def start_socket(self, value):
        # if we were assign to some socket before, delete us from the socket
    ...
    def updatePositions(self):
        ...
        if self.end_socket is not None:
            ...
            self.grEdge.setDesitination(*end_pos)
            self.grEdge.update()
        else:
            self.grEdge.setDesitination(*source_pos)
    ...
    def remove(self):
        if DEBUG: ("# Remvoing Edge", self)
        if DEBUG: ("remove edge from all sockets")
        self.remove_from_sockets()
        if DEBUG: ("remove grDedge")
        self.scene.grScene.removeItem(self.grEdge)
        self.grEdge = None
        if DEBUG: ("remove edge from scene")
        try:
            self.scene.removeEdge(self)
        except ValueError:
            pass
        if DEBUG: (" - everything is done")
