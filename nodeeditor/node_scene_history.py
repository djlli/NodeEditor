...
class SceneHistory():
    ...
    def restoreHistory(self):
        if DEBUG: print("Restoring history .... curennt_step: @%d" % self.history_current_step,
                        "(%d)" % len(self.history_stack))
        self.restoreHistoryStamp(self.history_stack[self.history_current_step])

    def storeHistory(self, desc, setModified=False):
        if setModified:
            self.scene.has_been_modified
        ...
        # if the pointer history_current_step is not at the end of the history_stack
        if self.history_current_step +1 < len(self.history_stack):
